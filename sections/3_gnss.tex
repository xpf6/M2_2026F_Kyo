...
\subsection{NLOS外れ値発生直後の1サンプル注入を防ぐ設計}
...
そこで本研究では，NLOS由来の測位逸脱が疑われる走行ログをケーススタディとして詳細に解析し，
品質監視の有効性を主として定性的に示す．
...
\subsection{評価対象ログとNLOS起因の測位逸脱の観測}
...
\begin{figure}[t]
  ...
  \begin{subfigure}[t]{0.32\linewidth}
    ...
    \caption{Naive fusion (with NLOS-induced GNSS outliers)}
    \label{fig:map_naive_nlos}
  \end{subfigure}
  \hfill
  \begin{subfigure}[t]{0.32\linewidth}
    \centering
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.75\linewidth]{figures/3/LIO_NLOS_FIX_part.png}
