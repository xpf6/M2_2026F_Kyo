...
遮蔽，マルチパス，および差分補正の途絶に起因して，まれに HMI を含む観測が発生する可能性がある．
このとき，誤拘束が1サンプルでもRTAB-Map注入されると，地図作成に劣化を引き起こす場合がある．
したがって本研究では，GNSS出力の重みを段階的に調整するのではなく，
採用または遮断によってRTAB-Mapへの拘束注入を制御する．

\subsection{入出力と二値ポリシー}
...
HMIが共分散を過小に偽装し得る点を踏まえ，本研究では遮断時に倍率拡大による重み調整へ依存せず，
...
\subsection{可用性スクリーニング}
\label{subsec:availability_gate}
可用性スクリーニングは，LIO等の外部推定に依存せず，
...
\subsection{完全性監視：短時間増分整合性に基づく遮断判定}
\label{subsec:integrity_gate}
完全性監視は，受信機状態が良好に見えるにもかかわらず外れ値を含むHMIを抑制するための機構である．
...
攻撃開始直後の最初の観測がバックエンド最適化へ注入される確率を低減できる．
...
HMIは発生頻度が低く，再現性のある収集が困難であった．
そこで本研究では，HMIが観測された走行ログをケーススタディとして詳細に解析し，
品質監視の有効性を主として定性的に示す．
...
\subsection{評価対象ログとHMIの観測}
\label{subsec:hmi_observation}
記録したGNSS観測（\texttt{/fix}）をFoxglove上で可視化したところ，
本来連続であるべき走行軌跡に対し，短時間の不連続（ジャンプ／断裂）に相当する挙動が確認された．
Fig.~\ref{fig:foxglove_hmi}に，異常挙動が観測された2日分（2025-10-23，2025-10-24）の例を示す．
10/23のログでは走行途中で不自然な横方向の逸脱が現れており，本研究の評価対象ログとして用いる．
一方，10/24のログでも同様の挙動が観測されたが，走行終盤での発生であり，
以降の区間が短く最適化過程の比較に適さないため，本節では「同様の現象が複数日に渡って観測され得る」ことの補助的証拠として提示するに留める．
なお，本研究では「受信機状態が良好に見える場合であっても測位が大きく偏る外れ値混入」をHMIとして扱うため，
当該区間をHMIが疑われる区間として解析対象に設定した．
...
\begin{figure}[t]
  ...
  \begin{subfigure}[t]{0.49\linewidth}
    \centering
    \includegraphics[width=\linewidth]{figures/3/10_24.png}
    \caption{2025-10-24: overview (reference only)}
    \label{fig:foxglove_1024_overview}
  \end{subfigure}
  \hfill
  \begin{subfigure}[t]{0.49\linewidth}
    \centering
    \includegraphics[width=\linewidth]{figures/3/10_24_0.png}
    \caption{2025-10-24: zoomed view around the anomaly}
    \label{fig:foxglove_1024_zoom}
  \end{subfigure}
  \caption{Examples of GNSS trajectory discontinuities visualized in Foxglove (topic: \texttt{/fix})}
  \label{fig:foxglove_hmi}
\end{figure}
...
Fig.~\ref{fig:map_compare_three}に，各手法で生成された点群地図と推定軌跡を示す
（白：点群地図，黄：推定軌跡）．
Naive fusionではHMIが疑われる区間において誤ったGNSS拘束が因子グラフへ注入され，
...
\begin{figure}[t]
  ...
  \begin{subfigure}[t]{0.32\linewidth}
    ...
    \caption{LIO only}
    \label{fig:map_only_lio}
  \end{subfigure}
  \hfill
  \begin{subfigure}[t]{0.32\linewidth}
    \centering
    \includegraphics[width=\linewidth]{figures/3/LIO_HMI_FIX_all.png}
    \caption{Naive fusion (with HMI)}
    \label{fig:map_naive_hmi}
  \end{subfigure}
  \hfill
  \begin{subfigure}[t]{0.32\linewidth}
    \centering
    \includegraphics[width=\linewidth]{figures/3/LIO+proposed.png}
    \caption{Proposed (GNSS Supervisor)}
    \label{fig:map_proposed}
  \end{subfigure}
  \caption{Qualitative comparison of mapping results (white: point cloud map, yellow: estimated trajectory)}
  \label{fig:map_compare_three}
\end{figure}

\subsection{HMI区間における挙動の詳細}
\label{subsec:hmi_detail}
HMIの影響をより明確に示すため，Naive fusionにおいて推定が不安定化した区間の例を
...
HMIが疑われる観測を事前に遮断し，最適化へ投入しないことが重要である．

\begin{figure}[t]
  \centering
  \includegraphics[width=0.75\linewidth]{figures/3/LIO_HMI_FIX_part.png}
  \caption{Example of local distortion observed after the anomaly in naive fusion (partial view)}
...
2025-10-23ログに対するNIS（中央値）と最終判定（PASS/FAIL）の時系列をFig.~\ref{fig:timeline_1023}に示す．
図中の破線は判定閾値$\gamma_{\mathrm{pass}}$を表し，上部のバーは最終的な採否（Accept/Reject）を示す．
異常区間ではNISが急増し，GNSS観測がRejectとして遮断されていることが分かる．
また，NISが閾値未満であっても，可用性判定（例：fix品質，補助状態量の鮮度）や
...
本ケーススタディにより，実環境においてHMIが疑われるGNSS観測が発生し得ること，
および品質監視により誤拘束の注入を抑制することで地図整合性が改善されることを確認した．
一方で，本環境ではHMIの発生頻度が低く，統計的な再現実験を行うには追加の長時間収集が必要である．
