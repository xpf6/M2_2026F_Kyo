...
/**
  ...
  * @note   DMAモードでは、この関数は指定された数のデータ転送がすべて完了した後に一度だけ呼び出される。
  * @param  hadc: ADCハンドル
  * @retval None
  */
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef* hadc)
{
  ...
  {
	  // ハードウェアが自動的に288回のサンプリングと転送を完了したので、メインループに通知するためのフラグを設定する
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
    ...
    //--------------------------------------------------------------------------//
    // 2. TRG信号の監視を開始 (TIM3の入力キャプチャで)
    //    これにより、TIM3が外部TRG信号に応答を開始する
    //--------------------------------------------------------------------------//
    if (HAL_TIM_IC_Start(&htim3, TIM_CHANNEL_1) != HAL_OK) {
        Error_Handler();
    }

    HAL_UART_Transmit(&huart2, (uint8_t*)"Spectrometer Ready (Full-Hardware-Trigger Mode).\r\n", 49, HAL_MAX_DELAY);
  /* USER CODE END 2 */
  ...
  /* USER CODE BEGIN WHILE */
    ...
    /* USER CODE BEGIN 3 */
    	// 3. スペクトル収集サイクルを1回トリガーする
      HAL_GPIO_WritePin(ST_GPIO_Port, ST_Pin, GPIO_PIN_SET);
      HAL_Delay(10); // 積分時間を10msに設定（必要に応じて調整可能）
      HAL_GPIO_WritePin(ST_GPIO_Port, ST_Pin, GPIO_PIN_RESET);

      // 4. ADCとDMAを起動。ハードウェアは自動的にTIM3から転送されるTRG信号を288回待機する
